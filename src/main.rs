                                    ...
                                    for element in elements.iter() {
                                        match element {
                                            ReceiptElement::Text {
                                                ...
                                                let char_width = effective_width
                                                    / printer_chars_per_line as f32;
...
fn render_raster_image(
    ...
    let scale_factor = if width > 300 || height > 150 {
        1.0 // Large images (logos) - display at actual size
    } else {
        3.0 // Small images (text) - scale up for readability
    };
    ...
    match QrCode::new(data.as_bytes()) {
        Ok(qr) => {
            ...
            let base_x = match alignment {
                Alignment::Left => 0.0,
                Alignment::Center => {
                    area_offset + (effective_width - pixel_size as f32) / 2.0
                }
